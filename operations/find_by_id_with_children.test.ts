...
describe('findByIdWithChildren', () => {
  ...
  const allCollections: Collection[] = [rootCollection, addressesCollection, profilesCollection, teamsCollection];
  ...
  it('should fail if one of the specified child collections does not have the root as its parent', async () => {
    ...
    await expect(findByIdWithChildren(ctx, 'users', 'testid', ['addresses', 'teams']))
  ...
  it('should limit itself to just the specified child collections', async () => {
    const dc = createDynamoMock('query', {});
    const ctx = createContext(dc, allCollections);
    await findByIdWithChildren(ctx, 'users', 'testid', ['profiles'], undefined, { scanForward: false });
    expect(dc.query).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('should search against all the known child collections if none are specified', async () => {
    ...
    expect(dc.query).toHaveBeenCalledWith(expect.objectContaining({
      KeyConditionExpression: 'pk = :value0 AND sk BETWEEN :value1 AND :value2',
  ...
  it('should return an empty root object if not found in the results', async () => {
    ...
    const dc = createDynamoMock('query', { 
      Items: [Converter.marshall(toWrapped({ ...addressesCollection, wrapperExtractKeys: [] } as CollectionDefinition, address1))],
    });
    const ctx = createContext(dc, allCollections);
  ...
  it('should fill out the root if found in the results', async () => {
    ...
    const rootDefinition = { ...rootCollection, wrapperExtractKeys: [] } as CollectionDefinition;
    const addressesDefinition = { ...addressesCollection, wrapperExtractKeys: [] } as CollectionDefinition;
    const dc = createDynamoMock('query', { 
      Items: [
