...
import { createContext, insert, findById, find, findByIdWithChildren } from '../dist';
...
async function main(): Promise<void> {
  ...
  const ctx = createContext(ddb, [usersCollection, credentialsCollection, addressesCollection]);
  ...
  const user1 = await insert(ctx, USERS_COLLECTION, { type: 'administrator', group: 'red', name: 'Joyce Mannheim' });
  console.log('created user 1', user1);
  const address1 = await insert(ctx, ADDRESSES_COLLECTION, { userId: user1._id, type: 'home', line1: '10 Home Place', suburb: 'HomeTown' });
  const address2 = await insert(ctx, ADDRESSES_COLLECTION, { userId: user1._id, type: 'work', line1: '78 Work Close', suburb: 'CityVillage' });
  const credentials1 = await insert(ctx, CREDENTIALS_COLLECTION, { userId: user1._id, type: 'password', password: 'testpassword' });
  const credentials2 = await insert(ctx, CREDENTIALS_COLLECTION, { userId: user1._id, type: 'google', googleUser: 'joyce.mannheim@gmail.com' });

  const user2 = await insert(ctx, USERS_COLLECTION, { type: 'guest', group: 'blue', name: 'Bill Langham' });
  console.log('created user 2', user2);
  const address3 = await insert(ctx, ADDRESSES_COLLECTION, { userId: user2._id, type: 'home', line1: '62 Local St', suburb: 'Daceyville' });
  const credentials3 = await insert(ctx, CREDENTIALS_COLLECTION, { userId: user2._id, type: 'password', password: '12345' });
  const credentials4 = await insert(ctx, CREDENTIALS_COLLECTION, { userId: user2._id, type: 'google', googleUser: 'billham@gmail.com' });
  ...
  // 2. Get a user's credential by ID and type
  console.log('user 2 password', await find(ctx, CREDENTIALS_COLLECTION, { userId: user2._id, type: 'password' }));
  ...
  // 3. Get a user with their addresses objects
  const result1 = await findByIdWithChildren(ctx, USERS_COLLECTION, user1._id, [ADDRESSES_COLLECTION], undefined, { scanForward: true }); // scan forward to get user object first
  console.log('user1 addresses', result1.root?.name, result1.children?.[ADDRESSES_COLLECTION], JSON.stringify(result1));
  
  // 4. Get a user with their credentials objects
  const result2 = await findByIdWithChildren(ctx, USERS_COLLECTION, user2._id, [CREDENTIALS_COLLECTION], undefined, { scanForward: false }); // scan backward to get user object first
  console.log('user2 credentials', result2.root?.name, result2.children?.[CREDENTIALS_COLLECTION], JSON.stringify(result2));
}
